...
(* let draw_names coord name = set_color black; match coord with | a, b
   when (a, b) = botleft_coord_of_botright -> moveto (a + (sq_height /
   10)) (b + (3 * sq_height / 5)); draw_and_move (String.split_on_char '
   ' name) (a + (sq_height / 10), b + (3 * sq_height / 5)) | a, b when
   (a, b) = botleft_coord -> moveto (a + (sq_height / 10)) (b + (3 *
   sq_height / 5)); draw_and_move (String.split_on_char ' ' name) (a +
   (sq_height / 10), b + (3 * sq_height / 5)) | a, b when (a, b) =
   botleft_coord_of_topleft -> moveto (a + (sq_height / 10)) (b + (3 *
   sq_height / 5)); draw_and_move (String.split_on_char ' ' name) (a +
   (sq_height / 10), b + (3 * sq_height / 5)) | a, b when (a, b) =
   botleft_coord_of_topright -> moveto (a + (sq_height / 10)) (b + (3 *
   sq_height / 5)); draw_and_move (String.split_on_char ' ' name) (a +
   (sq_height / 10), b + (3 * sq_height / 5)) | a, b when b = botlefty
   -> moveto (a + (sq_width / 10)) (b + (3 * sq_height / 5));
   draw_and_move (String.split_on_char ' ' name) (a + (sq_height / 10),
   b + (3 * sq_height / 5)) | a, b when a = botleftx -> moveto (a +
   (sq_height / 10)) (b + (2 * sq_width / 5)); draw_and_move
   (String.split_on_char ' ' name) (a + (sq_height / 10), b + (2 *
   sq_height / 5)) | a, b when b = temp -> moveto (a + (sq_width / 10))
   (b + (3 * sq_height / 5)); draw_and_move (String.split_on_char ' '
   name) (a + (sq_height / 10), b + (3 * sq_height / 5)) | a, b when a =
   temp1 -> moveto (a + (sq_height / 10)) (b + (2 * sq_width / 5));
   draw_and_move (String.split_on_char ' ' name) (a + (sq_height / 10),
   b + (2 * sq_height / 5)) | a, b -> () *)
...
let draw_colors color coord =
  ...
  | Some (r, g, b) -> (
      ...
      | x, y when y = botlefty ->
          set_color (rgb r g b);
          fill_rect (x + 1)
            (*changed from x+2 to just x*) (y + (4 * sq_height / 5))
            (sq_width - 3)
            ((sq_height / 5) - 1)
          (* changed -4 to 0*)
      | x, y when x = botleftx ->
...
let topright_of_botleft (x, y) =
  match (x, y) with
  | a, b when (a, b) = botleft_coord_of_botright ->
      (a + sq_height, b + sq_height)
  | a, b when (a, b) = botleft_coord -> (a + sq_height, b + sq_height)
  | a, b when (a, b) = botleft_coord_of_topleft ->
      (a + sq_height, b + sq_height)
  | a, b when (a, b) = botleft_coord_of_topright ->
      (a + sq_height, b + sq_height)
  | a, b when a = getx botleft_coord -> (a + sq_height, b + sq_width)
  | a, b when b = gety botleft_coord -> (a + sq_width, b + sq_height)
  | a, b when a = getx botleft_coord_of_topright ->
      (a + sq_height, b + sq_width)
  | a, b when b = gety botleft_coord_of_topright ->
      (a + sq_width, b + sq_height)
  | _ -> failwith "not a proper location"

let get_top_right lst = List.map topright_of_botleft lst

let bl = coords_list

let tr = get_top_right bl

let in_the_square (x, y) (x1, y1) (x2, y2) =
  match (x, y) with
  | x, y when x >= x1 && x <= x2 && y >= y1 && y <= y2 -> true
  | _ -> false

(* let find_hover bl_lst tr_lst = List.filter (in_the_square mouse_pos) *)
